#!/bin/bash
# 누락된 fromClientId 컬럼 추가 스크립트

set -euo pipefail

echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "🔧 누락된 fromClientId 컬럼 추가"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
COLUMN_EXISTS=$(docker compose exec -T db psql -U app -d ggfinder -tAc \
  "SELECT EXISTS (
    ...
    WHERE table_name='favorites' AND column_name='fromClientId'
  );" 2>/dev/null || echo "false")

if [ "$COLUMN_EXISTS" = "t" ]; then
  echo "✅ fromClientId 컬럼이 이미 존재합니다."
else
  echo "⚠️  fromClientId 컬럼이 없습니다. 추가 중..."
  
  # 컬럼 추가
  docker compose exec -T db psql -U app -d ggfinder -c \
    "ALTER TABLE \"favorites\" ADD COLUMN IF NOT EXISTS \"fromClientId\" INTEGER;" 2>&1
  
  if [ $? -eq 0 ]; then
    echo "✅ fromClientId 컬럼 추가 완료"
    
    # 외래 키 제약 조건 추가 (이미 존재하면 무시)
    echo "📋 외래 키 제약 조건 추가 중..."
    docker compose exec -T db psql -U app -d ggfinder -c \
      "DO \$\$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM pg_constraint 
          WHERE conname = 'favorites_fromClientId_fkey'
        ) THEN
          ALTER TABLE \"favorites\" 
          ADD CONSTRAINT \"favorites_fromClientId_fkey\" 
          FOREIGN KEY (\"fromClientId\") 
          REFERENCES \"clients\"(\"id\") 
          ON DELETE SET NULL ON UPDATE CASCADE;
        END IF;
      END \$\$;" 2>&1 || echo "⚠️  외래 키 추가 실패 (이미 존재할 수 있음)"
    
    # 인덱스 추가 (이미 존재하면 무시)
    echo "📋 인덱스 추가 중..."
    docker compose exec -T db psql -U app -d ggfinder -c \
      "CREATE INDEX IF NOT EXISTS \"favorites_fromClientId_idx\" ON \"favorites\"(\"fromClientId\");" 2>&1
    
    echo "✅ fromClientId 컬럼 및 관련 제약 조건 추가 완료"
  else
    echo "❌ 컬럼 추가 실패"
    exit 1
  fi
...
if [ "$FINAL_CHECK" = "t" ]; then
  echo "✅ fromClientId 컬럼 확인 완료"
  
  # 외래 키 확인
  FK_CHECK=$(docker compose exec -T db psql -U app -d ggfinder -tAc \
    "SELECT COUNT(*) FROM pg_constraint WHERE conname = 'favorites_fromClientId_fkey';" 2>/dev/null || echo "0")
  
  if [ "$FK_CHECK" = "1" ]; then
    echo "✅ 외래 키 제약 조건 확인 완료"
  else
    echo "⚠️  외래 키 제약 조건이 없습니다."
  fi
else
  echo "❌ fromClientId 컬럼이 여전히 없습니다."
  exit 1
fi
