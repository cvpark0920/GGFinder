# finder.ggacademy.top 서브도메인 설정 가이드

## 🎯 목표
`finder.ggacademy.top` 서브도메인을 DigitalOcean Droplet에 연결하고 SSL 인증서를 적용합니다.
...
메인 도메인(`ggacademy.top`)이 이미 A 레코드로 설정되어 있다면:
...
# 또는 온라인 도구 사용
# https://dnschecker.org/#A/finder.ggacademy.top
```
...
```bash
# HTTP 확인
curl -I http://finder.ggacademy.top

# HTTPS 확인
curl -I https://finder.ggacademy.top

# SSL 인증서 확인
openssl s_client -connect finder.ggacademy.top:443 -servername finder.ggacademy.top
```
...
- 서브도메인이 Droplet IP로 올바르게 설정되었는지 확인
...
- [ ] DNS 레코드 추가 완료 (finder.ggacademy.top → Droplet IP)
- [ ] DNS 전파 확인 완료
- [ ] Nginx 설치 및 설정 완료
- [ ] SSL 인증서 발급 완료
...
1. ✅ `https://finder.ggacademy.top` 접속 가능
2. ✅ SSL 인증서 정상 작동 (자물쇠 아이콘)
3. ✅ 애플리케이션 정상 작동
4. ✅ API 호출 정상 작동

## 💡 서브도메인 vs 메인 도메인

서브도메인(`finder.ggacademy.top`)을 사용하는 경우:

**장점:**
- 메인 도메인(`ggacademy.top`)을 다른 용도로 사용 가능
- 여러 서브도메인으로 서비스 분리 가능
- 관리가 용이

**DNS 설정:**
- A 레코드: `finder` → `<DROPLET_IP>` (권장)
- 또는 CNAME: `finder` → `@` (메인 도메인이 A 레코드로 설정된 경우)
